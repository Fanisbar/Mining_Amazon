...
* **Optimization:** Both notebooks include an automatic download script. You can use the `max_rows` variable at the beginning of the notebooks to limit the dataset size and save system RAM/execution time during testing. (Warning: limiting max_rows may affect the quality of results as the metadata and reviews CSV's may refer to different products, potentially leading to missing product names)
