...
\begin{itemize}
    \item After downloading, the datasets are used to create pandas DataFrames that can be later manipulated as desired.
...
Briefly explore the nature of the datasets. The notebook displays the average product price in each category. This happens in between cells of code that aim to replace all bad values in the 'price' column of a category, with that mean price value. A set of plots then show the distribution of ratings in each category within the 5-star range. In simple words, it shows how many 1-star ratings, how many 2-star ratings, how many 3-star ratings etc. there are in each category. Next, 2 plots per category follow, showcasing the distribution of review lengths both in words and in characters. Explain what information we obtain from those plots

\subsection{Product Performance Analysis}
Here we find the best and worst sellers
